...
function convert(data, res) { // uploaded data : params + gpx/fit data in base64 format
    let rootdir = 'storage/gpx/'
    ...
    if (params.hasOwnProperty('name')) {
        ...
        if (params.name.endsWith('_log_2.csv') || params.name == '2501580_out.csv') {
            fs.writeFileSync(file, Buffer.from(data, 'base64').toString()) // save in uploads
            sendres(params, 'ok')
            return
        }
        ...
        if (params.name.endsWith('.csv')) {
            let csv_data = Buffer.from(data, 'base64').toString()
            if (params.saveuploads == 'true') fs.writeFileSync(file, fit_data)
