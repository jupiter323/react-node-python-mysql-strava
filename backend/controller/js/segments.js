...
function getDarkSkyData(params, trackinfo, callback){
    ...
    //    fs.writeFileSync(__dirname+'./../weather-files/darksky_url.txt', url);
    var request = https.get(url, (resp) => {
        ...
        resp.on('end', () => {
            ...
            if (params.saveweatherfiles=='true') {
                ...
                try {fs.mkdirSync(__dirname+'./../weather-files/')} catch(err){}
                try {
                    fs.writeFileSync(__dirname+'./../weather-files/'+ name + '_' + formatdate(d) + '.json', json);
                }
                catch(err) { 
