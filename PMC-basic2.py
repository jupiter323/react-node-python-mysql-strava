...
all_training_files = [s.replace("final_data/User 2 - 100 meter - added variables - traintest - new/User 2/Train data\\", "") for s in all_training_files]
print('Training Files : \n', all_training_files)
all_testing_files = glob.glob(main_path + user + test_data_path + "*.csv")
all_testing_files = [s.replace("final_data/User 2 - 100 meter - added variables - traintest - new/User 2/Test data\\", "") for s in all_testing_files]
print('Testing Files : \n', all_testing_files)
...
for i in range(len(combined_inp_list)):
    ...
    X = trainDF.drop(['hrCategory', 'hrCategory1', 'shape', 'activity_id', 'm_v', 'intensityscore',
                      'ftpreal20min', 'ftpreal10min', 'ftpreal5min', 'weight', 'user_id' , 'length','dist','age'], axis=1)
    ...
    with open('user_combinations_models/'+ user[55:] + '/' + '-'.join(names_list) + '_final_tuned_model.pkl' , 'wb') as fid:
...
for file__ in glob.glob('user_combinations_models/' + user[55:] + '/' + "*final_tuned_model.pkl"):
    ...
    for file in glob.glob(main_path + user + test_data_path + "*.csv"):
        ...
        testDF.drop(['index','time', 'lat', 'lon', 'error', 'beats', 'beatcount', 'time1', 'gpxtimestamp'], inplace=True, axis=1)
